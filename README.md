...
> Это **каркас**: страницы созданы как пустые шаблоны с корректными URL, мета-тегами, разметкой, sitemap/robots, аналитикой и рабочими формами заявок (лиды уходят в Bitrix24). Контентное наполнение — отдельный этап (через Decap CMS).
...
| `npm run server`    | Бэкенд-обработчик заявок (порт 3000) → лиды в Bitrix24 CRM.                  |
...
```
server/           бэкенд-обработчик заявок → Bitrix24 CRM (порт 3000)
src/
  _includes/
...
## Формы и заявки в CRM (Bitrix24)

Заявки со всех форм сайта уходят в Bitrix24 через собственный бэкенд-обработчик в каталоге `server/` — без сторонних сервисов и без зависимостей (только Node).
...
3. `npm run dev` (или `npm run server` отдельно) — и формы работают.
