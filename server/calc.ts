 ...
 *
 * Города: калькулятор считает только до Москвы — тариф на другие направления
 * зависит от плеча по России и считается менеджером по заявке.
 *
...
export const CALC_CONFIG = {
  /**
   * Города назначения, для которых калькулятор считает сам.
   * Пока это только Москва: тарифы на другие направления зависят от плеча по
   * России и считаются менеджером — на странице об этом сказано отдельно.
   * Добавляете город — не забудьте про его тариф в `computeDelivery`.
   */
  cities: ["Москва"] as string[],
  ...
  /**
   * Комиссия ТЛК БАРС — фиксированная сумма, зависит от стоимости товара.
